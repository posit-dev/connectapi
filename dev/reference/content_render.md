...
Submit a request to render a content item. Once submitted, the server
runs an asynchronous process to render the content. This might be useful
if content needs to be updated after its source data has changed,
...
- variant_key:
  ...
  If a variant key is provided, render that variant. Otherwise, render
  the default variant.
...
object that can be used to track completion of the render.
