...
test_that("content_set_all_integrations() sends expected request", {
  with_mock_dir("2024.12.0", {
    client <- Connect$new(server = "https://connect.example", api_key = "fake")
    x <- content_item(client, "12345678")
    y <- integration(client, "f8688548")
  })
  without_internet(
    expect_PUT(
      content_set_all_integrations(x, y),
...
test_that("content_set_all_integrations() fails when provided the wrong class", {
  with_mock_dir("2024.12.0", {
    client <- Connect$new(server = "https://connect.example", api_key = "fake")
    x <- content_item(client, "12345678")
  })
  expect_error(
    content_set_all_integrations(x, "string"),
    "`integrations` must be a `connect_integration` class object or a list"
  )
    expect_error(
    content_set_all_integrations(x, list("string")),
