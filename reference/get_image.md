...
`get_image` saves the content image to the given path (default: temp
file). `delete_image` removes the image (optionally saving to the given
path) `has_image` returns whether the content has an image
